...
namespace ChatApp
{
    internal class ChatClient : IDisposable
    {
        ...
        public async Task SendMessage(string message)
        {
            ...
            await _client.SendAsync(Encoding.UTF8.GetBytes(msg), WebSocketMessageType.Text, true, CancellationToken.None);
        }

        private async Task SubscribeToChannel(string channel)
        {
            await _client.SendAsync(Encoding.UTF8.GetBytes(channel), WebSocketMessageType.Text, true, CancellationToken.None);
        }
